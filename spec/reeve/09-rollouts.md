# reeve spec — Staged Rollouts (REV-008)
...
(08-packaging §10.2, pull-only from below) — makes staging natural: a device's
...
information the server already has (05-health-journal §7 health, 02-channel §4.3 presence, Margo
deployment status).
...
selection, or a LABEL selection (docs/decisions/tree-render.md D12 — labels group,
...
- A rollout is defined by: the source tree revision (of the layer
  ...
  threshold.
- **Convergence target (normative, docs/decisions/tree-render.md D12):** a device's
...
- Starting a wave means: for each device in the wave, advance that
  ...
  tree revision (bumping manifestVersion, 08-packaging §10.2). Nudges (02-channel §4.4)
...
- **Health** (05-health-journal §7.4): classification is healthy — specifically not
  device-degraded; link-degraded/unknown devices count per the
  offline policy below.
- **Presence** (02-channel §4.3) where available: distinguishes "converged and
...
- The rollout engine MUST NOT move any device manifest backward on
  ...
  unreachable device that later backfills healthy history (05-health-journal §7.4)
...
- Rollback is a NEW rollout whose source is the prior tree revision
  ...
  (08-packaging §10.2 anti-rollback) even when the CONTENT reverts.

### 11.6 Observability

Rollout and wave transitions (`started`, `gated`, `paused`,
`completed`, `failed`) are published as `rollout` events (04-status-stream §6.3) and
exposed in the REST API for the UI's rollout views (per-device:
advanced / converged / healthy / undetermined / failed).

### 11.7 Federation

Single-writer-per-layer (06-federation §8.4) holds unchanged: a rollout is
...
- Creating, resuming, and aborting rollouts MUST be authorized,
  ...
  agent self-update (08-packaging §10.5) — reach the fleet.
- Auto-pause thresholds are a safety mechanism, not a security
  boundary: a device can lie about its own health (05-health-journal §7.6) and
...
- Pause-not-rollback means a bad-but-not-failing commit stays
  deployed on early waves until humans act; the audit trail and
  04-status-stream §6 events exist to make that window short.
