# reeve spec — Live Status Stream (REV-003)
...
bidirectional — the terminal (03-terminal Section 5.1). Events tell the UI
...
- `GET /api/reeve/v1/events` — `text/event-stream`, same listener
  as everything else (08-packaging Section 10.2). MUST NOT be served
...
- Consumers are browsers/UI clients. reeve-agent MUST NOT depend on
  this endpoint (agents have 02-channel Section 4; Law 5 forbids agent
...
| `device-presence` | 02-channel §4.3 | `deviceId`, `state` (`online`\|`offline`), `since` | channel opens/drops |
...
| `terminal-session` | 03-terminal §5.4 | `sessionId`, `deviceId`, `phase` (`requested`\|`opened`\|`closed`\|`denied`), `user` | session lifecycle transition |
| `health-state` | 05-health-journal §7.4 | `deviceId`, `state` (`healthy`\|`degraded`\|`unknown`), `kind` (`device`\|`link`) | health classification changes |
| `verify-restore` | 07-durability §9.4 | `outcome` (`ok`\|`failed`), `snapshotTs`, `detail` | a verify-restore run completes |
| `durability-lag` | 07-durability §9.3 | `generation`, `lastSeq`, `lagSeconds` | changeset upload lag crosses/clears a threshold — ops dashboard signal |
| `rollout` | 09-rollouts §11.6 | `rolloutId`, `wave`, `phase` (`started`\|`gated`\|`paused`\|`completed`\|`failed`) | rollout/wave transition |
| `secret-rotation` | 10-secrets §12 | `secretName`, `scope`, `version`, `state` (`propagating`\|`converged`) | a secret version changes / all affected devices report converged |
...
- `terminal-session` events expose session metadata (who, which
  device, when) — intentional audit visibility per 03-terminal Section 5.4;
